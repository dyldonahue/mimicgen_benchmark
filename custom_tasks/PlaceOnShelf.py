''' Place On Shelf Task '''
#  Pick Up commplex objects and place them on a shelf
...
from custom_objects.objects import ShelfObject, MugObject

class PlaceOnShelfTask(SingleArmEnv):
    """
    ...
    def reward(self, action):
        ...
        Un-normalized components if using reward shaping:
            ...
            - Reaching: in [0, 1], to encourage the arm to reach the shelf and grasp it
            - Lifting: in {0, 0.5}, non-zero if arm has lifted the shelf above the table top by a margin
            - Aligning: in [0, 0.5], encourages moving shelf tip towards the screw thread
            - Touching: in {0, 2}, non-zero if the shelf tip is touching the screw head
    ...
    def _load_model(self):
        ...
        if self.placement_initializer is not None:
            ...
            # shelf placement is statically back corner of the table
            x_center = -self.table_full_size[0] / 2 #+ 0.05
            x_tol = 0.05  # tolerance for placement
            # add sampler for shelf placement
            # shelf is placed at the back corner of the table, with a tolerance in x direction
            # y is always 0, and rotation is not needed
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def shelf_quat(obs_cache):
                return convert_quat(np.array(self.sim.data.body_xquat[self.shelf_body_id]), to="xyzw")
    ...
    def _check_success(self):
        """
        Check if shelf is correctly touching the screw head.

        Returns:
            bool: True if shelf is touching the screw head, False otherwise.
        """
        _ = self.staged_rewards()
    ...
    def visualize(self, vis_settings):
        ...
        if vis_settings["grippers"]:
            self._visualize_gripper_to_target(gripper=self.robots[0].gripper, target=self.shelf)