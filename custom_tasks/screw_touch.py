...
from custom_objects.objects import (
    ScrewDriverObject,
    ScrewWithInsert,
    ScrewObject,
    ThreadedInsertObject,
)


class screw_touch_d1(SingleArmEnv):
    """
    ...
    def _load_model(self):
        ...
        self.screwdriver = ScrewDriverObject(name="screwdriver")

        self.threaded_insert = ThreadedInsertObject(name="threaded_insert")

        self.screw = ScrewObject(name="screw")
        ...
        self.placement_initializer.append_sampler(
            UniformRandomSampler(
                ...
                x_range=[
                    -self.table_full_size[0] / 2 * 0.3,
                    +self.table_full_size[0] / 2 * 0.3,
                ],
                y_range=[
                    -self.table_full_size[1] / 2 * 0.4,
                    +self.table_full_size[1] / 2 * 0.4,
                ],
                rotation_axis="z",
                rotation=np.pi,
    ...
    def _setup_references(self):
        ...
        self.screwdriver_body_id = self.sim.model.body_name2id(
            self.screwdriver.root_body
        )
        self.screwinsert_body_id = self.sim.model.body_name2id(
            self.screwinsert.root_body
        )
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset:
            ...
            # freeze it so it doesnt tip over
            body_id = self.sim.model.body_name2id(
                "screw_with_insert_threaded_insert_main"
            ...
            object_placements["screw_with_insert"] = (
                insert_pos,
                insert_quat,
                self.screwinsert,
            )
            ...
            for obj_pos, obj_quat, obj in object_placements.values():
                ...
                if obj.joints and len(obj.joints) > 0:
                    ...
                    self.sim.model.body_quat[body_id] = convert_quat(
                        obj_quat, to="wxyz"
                    )
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def screwdriver_quat(obs_cache):
                return convert_quat(
                    np.array(self.sim.data.body_xquat[self.screwdriver_body_id]),
                    to="xyzw",
                )
            ...
            def screwinsert_quat(obs_cache):
                return convert_quat(
                    np.array(self.sim.data.body_xquat[self.screwinsert_body_id]),
                    to="xyzw",
                )
            ...
            sensors = [
                ...
                screwinsert_pos,
                screwinsert_quat,
                gripper_to_screwdriver,
                gripper_to_screwinsert,
                screwdriver_to_screwinsert,
            ]
    ...
    def _check_lift(self):
        ...
        if not self._check_grasp(
            gripper=self.robots[0].gripper, object_geoms=self.screwdriver
        ):
            return False
    ...
    def _check_align(self):
        ...
        if self._check_lift() >= 4:
            ...
            if (
                distance[2] > 0.008
                and abs(distance[0]) < 0.018
                and abs(distance[1]) < 0.015
            ):
    ...
    def _check_contact(self):
        ...
        if (
            abs(distance[0]) < self._lower_tolerance_x
            and abs(distance[1]) < self._lower_tolerance_y
            and abs(distance[2]) < self._lower_tolerance_z
        ):
            return True
        elif (
            abs(distance[0]) < self._higher_tolerance_x
            and abs(distance[1]) < self._higher_tolerance_y
            and abs(distance[2]) < self._higher_tolerance_z
        ):
    ...
    def visualize(self, vis_settings):
        ...
        if vis_settings["grippers"]:
            self._visualize_gripper_to_target(
                gripper=self.robots[0].gripper, target=self.screwdriver
            )


class screw_touch_d2(screw_touch_d1):
    """
    Variation of ScrewTouch where the screw is randomly oriented in SO3 during
    environment reset.
    """
    ...
    def __init__(self, **kwargs):
        ...
        self.align_cos_threshold = np.cos(np.deg2rad(20))
    ...
    def _check_orientation(self):
        """
        Returns True if screwdriver is angled similarly to the screw angle.
        """
        ...
        if self._check_lift() >= 4:
            ...
            site_id = self.sim.model.site_name2id("screw_with_insert_screw_head_site")
            head_pos = self.sim.data.site_xpos[site_id]
            screw_pos = self.sim.data.body_xpos[self.screwinsert_body_id]

            # find angle between the axis and the vertical
            screw_axis = head_pos - screw_pos
            screw_axis = screw_axis / np.linalg.norm(screw_axis)

            screwdriver_axis = tip_pos - screwdriver_pos
            screwdriver_axis = screwdriver_axis / np.linalg.norm(screwdriver_axis)

            # invert because tip and head are oppositely directed
            cos_theta = -1 * np.dot(screwdriver_axis, screw_axis)

            if 0.935 <= cos_theta <= 1.0:
                return True
    ...
    def _check_align(self):
        """
        Returns True if screwdriver tip is near the screw head and aligned,
        but not touching.
        """
        ...
        if self._check_lift() >= 4:
            ...
            screwdriver_vec = tip_pos - screwdriver_pos
            screwdriver_axis = screwdriver_vec / np.linalg.norm(screwdriver_vec)

            cos_theta = -1 * np.dot(screwdriver_axis, screw_axis)

            # check if the tilted screwdriver is close to the screw axis
            length = np.dot(screwdriver_vec, screw_axis)
            distance = np.linalg.norm(tip_pos - head_pos)

            if 0.1 > distance > 0.01 and cos_theta > 0.935:
                return True
            else:
                return False
        else:
    ...
    def _check_contact(self):
        ...
        distance = np.linalg.norm(tip_pos - head_pos)
        # print("COS THETA:", np.round(cos_theta, 3), " DISTANCE:", np.round(distance, 4))
        dist = tip_pos - head_pos
        # print("DISTANCE:", np.round(dist, 4))

        if distance < 0.01 and cos_theta > 0.935:
            return True
        else:
            return False
