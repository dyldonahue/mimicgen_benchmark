...
class MugTreeTask(SingleArmEnv):
    """
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def tree_quat(obs_cache):
                return convert_quat(np.array(self.sim.data.body_xquat[self.tree_body_id]), to="xyzw")
    ...
    def _check_success(self):
        """
        Check if tree is correctly touching the screw head.

        Returns:
            bool: True if tree is touching the screw head, False otherwise.
        """
        _ = self.staged_rewards()
    ...
    def visualize(self, vis_settings):
        ...
        if vis_settings["grippers"]:
            self._visualize_gripper_to_target(gripper=self.robots[0].gripper, target=self.tree)